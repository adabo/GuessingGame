...
ScreenState g_next_screen_state = MAINMENU;
...
void StartGameLoop()
{
    ...
    while (!g_quit)
    {
        SetScreenState(/* g_next_screen_state */);
...
void RunScreenMainMenu()
{
    ...
    if      (g_player_input == 'R')
    {
        g_next_screen_state = RULES;
    }
    else if (g_player_input == 'P')
    {
        g_next_screen_state = DIFFICULTY;
    }
